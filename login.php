...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($query->rowCount() > 0) {
        ...
        // die();
        if ($checkPass) {
            setcookie("logins_id", $results->id, time() + (86400 * 365), '/');
            setcookie("logins_username",  $results->username, time() + (86400 * 365), '/');
            setcookie("logins_password", $results->password, time() + (86400 * 365), '/');
            setcookie("logins_name", $results->name, time() + (86400 * 365), '/');
            setcookie("logins_email", $results->email, time() + (86400 * 365), '/');
            setcookie("logins_phone", $results->phone, time() + (86400 * 365), '/');
            setcookie("logins_address", $results->address, time() + (86400 * 365), '/');
            setcookie("logins_status", $results->status, time() + (86400 * 365), '/');
